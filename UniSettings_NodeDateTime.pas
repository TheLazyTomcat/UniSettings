...
ReadLock;
try
  with TUNSNodeDateTime(CheckedLeafNodeTypeAccess(ValueName,vtBool,'DateTimeValueGet')) do
    If AccessDefVal then
      Result := Value
    else
      Result := DefaultValue;
...
WriteLock;
try
  with TUNSNodeDateTime(CheckedLeafNodeTypeAccess(ValueName,vtBool,'DateTimeValueSet')) do
    If AccessDefVal then
      Value := NewValue
    else
